...
# Load the dataset
file_path = r'C:\Users\srira\OneDrive\Desktop\Gen_AI\Training\AllDomains\1IndustrialPolicyDataset.py.csv'  # Update this path
...
for i, column in enumerate(numeric_data.columns): # type: ignore
    plt.subplot(2, (len(numeric_data.columns) + 1) // 2, i + 1) # type: ignore
    sns.boxplot(y=numeric_data[column]) # type: ignore
...
outlier_percentage = {}
for column in numeric_data.columns: # type: ignore
...
# Correlation heatmap
correlation_matrix = numeric_data.corr() # type: ignore
...
X = data.drop('Employment', axis=1)  # Replace 'Crunchiness' with the actual target variable name
y = data['Employment']  # Replace 'Crunchiness' with the actual target variable name
...
if 'Employment' in data.columns:
    y_logistic = data['Employment']  # Replace with the actual binary target variable
    print("Unique values in the target variable for Logistic Regression:", y_logistic.unique())
    X_logistic = data.drop('Employment', axis=1)
