...
# Ask user how to handle non-numeric columns
handle_non_numeric = input("Do you want to handle non-numeric columns? (yes/no): ").strip().lower()

if handle_non_numeric == 'yes':
    # Convert non-numeric columns to numeric if possible
    for column in data.select_dtypes(include=['object']).columns:
        # Ask user for a method to handle non-numeric columns
        method = input(f"Choose a method for column '{column}' (drop/encode): ").strip().lower()
        if method == 'drop':
            data.drop(column, axis=1, inplace=True)  # Drop the column
            ...
            print("Invalid method selected. No changes made to non-numeric columns.")
...
X = data.drop('loan_amount', axis=1)  # Replace 'target_column' with the actual target variable name
y = data['loan_amount']  # Replace 'target_column' with the actual target variable name
