...
# Load the dataset
file_path = r'C:\Users\srira\OneDrive\Desktop\Gen_AI\Training\AllDomains\taxifare\original_cleaned_nyc_taxi_data_2018.csv'  # Update this path
...
X = data.drop('total_amount', axis=1)  # Replace 'Crunchiness' with the actual target variable name
y = data['total_amount']  # Replace 'Crunchiness' with the actual target variable name
...
if 'total_amount' in data.columns:
    y_logistic = data['total_amount']  # Replace with the actual binary target variable
    print("Unique values in the target variable for Logistic Regression:", y_logistic.unique())
    X_logistic = data.drop('total_amount', axis=1)
...
# Clustering of Actual vs Predicted
combined_data = pd.DataFrame({'Actual': y_test, 'Predicted': y_pred_rf})
...
# Visualize Clusters
plt.figure(figsize=(10, 6))
sns.scatterplot(data=combined_data, x='Actual', y='Predicted', hue='Cluster', palette='viridis', alpha=0.6)
plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--', label='Perfect Prediction')
plt.xlabel('Actual')
plt.ylabel('Predicted')
plt.title('Clusters of Actual vs Predicted Values')
plt.legend()
plt.show()
