...
if __name__ == "__main__":
    ...
    if order == "rating":
        video_results = sorted(video_results, key=lambda x: x["Rating"], reverse=True)
    df_videos = pd.DataFrame(video_results)
    print(f"Top {max_results} Videos Found ({order}):")
    print(df_videos)
    filename = f"yt_channel_{order}_{channel_name.replace(' ', '_')}_{max_results}results.csv"
    df_videos.to_csv(filename, index=False, encoding="utf-8-sig")
