...
data['Economic_Loss($)'] = (data['Economic_Loss($)'] > data['Economic_Loss($)'].median()).astype(int)
...
# Separate features and target variable
# Ensure 'Economic_Loss($)' is in the data_encoded DataFrame
if 'Economic_Loss($)' in data_encoded.columns:
    X_logistic = data_encoded.drop(['Disaster_ID', 'Date', 'Economic_Loss($)', 'Economic_Loss($)'], axis=1)
else:
    print("Economic_Loss($) column not found in data_encoded.")
...
y_logistic = data['Economic_Loss($)']  # Use the original data for the target variable
...
# Create a suitable DataFrame for association rules
